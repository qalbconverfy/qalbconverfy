...
const handleUpload=()=>{
    ...
    return (
        <div className='w-full h-[100vh] flex flex-col items-center ' style={{ backgroundImage: 'url(/bgi.png)', backgroundSize: 'fixed', backgroundPosition: 'center' }}>
            <div className='w-full h-[80px]  flex items-center gap-[20px] px-[20px]'>
                <MdOutlineKeyboardBackspace className='text-white cursor-pointer w-[25px]  h-[25px] ' onClick={() => navigate(`/`)} />
                <h1 className='text-white text-[20px] font-semibold'>Upload Contents</h1>
            </div>

            <div className='w-[90%] max-w-[600px] h-[80px] bg-[white] rounded-[15px] flex justify-around items-center gap-[10px]' >

                <div className={`${uploadType == "post" ? "bg-gradient-to-r from-blue-700 via-black to-blue-700 text-white shadow-2xl shadow-black" : ""}  w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-[13px] hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={() => setUploadType("post")}>Post</div>

                <div className={`${uploadType == "story" ? "bg-gradient-to-r from-blue-700 via-black to-blue-700 text-white shadow-2xl shadow-black" : ""}  w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-[13px] hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={() => setUploadType("story")}>Story</div>

                <div className={`${uploadType == "loop" ? "bg-gradient-to-r from-blue-700 via-black to-blue-700 text-white shadow-2xl shadow-black" : ""}  w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-[13px] hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={() => setUploadType("loop")}>Loop</div>
            </div>

            {!frontendMedia && <div className='w-[80%] max-w-[500px] h-[250px] bg-[#0e1316] border-white border-2 flex flex-col items-center justify-center gap-[8px] mt-[15vh] rounded-2xl cursor-pointer hover:bg-transparent' onClick={() => mediaInput.current.click()}>
