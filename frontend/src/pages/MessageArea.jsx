...
<div className='w-[40px] h-[40px] border-2 border-black rounded-[7px] cursor-pointer overflow-hidden' onClick={()=>navigate(`/profile/${selectedUser.userName}`)}>
...
<form className='w-[90%] max-w-[800px] h-[80%] rounded-[10px] flex items-center gap-[10px] px-[20px] relative'style={{ backgroundImage: 'url(/bgi.png)', backgroundSize: 'fixed', backgroundPosition: 'center' }} onSubmit={handleSendMessage}>
  {frontendImage && <div className='w-[100px] rounded-[10px] h-[100px] absolute top-[-120px] right-[10px] overflow-hidden'>
  ...
  {(input || frontendImage) &&  <button className='w-[60px] h-[40px] rounded-full bg-transparent flex items-center justify-center cursor-pointer'><IoMdSend className='w-[25px] h-[25px] text-white'/></button>}
